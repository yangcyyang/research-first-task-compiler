...
**建议执行模型（可选元数据，非 gate）**：任务卡可附带一行「建议执行模型」，按任务能力缺口给匹配建议：调研/聚合型 → 联网检索能力强的模型；视觉/出图型 → 图像能力模型；代码验证/执行型 → 代码能力模型；默认 → 当前模型不切换。它是可跳过的元数据，不是确认门槛；用户可在方向闸门 1 确认时一并决定（如「确认任务卡，用 XX 模型」），或保持当前模型。若切换执行者，先读 workflow-state 文件再继续，避免上下文断裂。推荐映射随模型能力变化会过时，只作示例、不硬编码为规则。
